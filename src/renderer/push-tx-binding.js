// OP_PUSH_TX preimage binding.
//
// Vendored from the Runar compiler: packages/runar-compiler, source
// dist/passes/oppushtx-codegen.js, export CHECK_PREIMAGE_BINDING_HEX.
// That package is not a dependency of this repo, so the bytes are copied
// here. test/push-tx.test.js pins them to the export when it resolves.
//
// The run derives an ECDSA signature in script from hash256(preimage) and
// checks it against a fixed public key with OP_CHECKSIGVERIFY, so the
// preimage on the stack is the one the node signed. It takes no
// spender-supplied signature and contains no OP_CODESEPARATOR.
//
// Precondition: preimage on top of the stack.
// Postcondition: the same preimage on top. Net stack effect zero.
// The derived signature carries sighash 0x41 (ALL|FORKID), so the signed
// message is always BIP-143.

const CHECK_PREIMAGE_BINDING_HEX =
  '';

// Export for use in app
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CHECK_PREIMAGE_BINDING_HEX };
}

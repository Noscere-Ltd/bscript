...
import { InputLimits, CanonicalJsonError } from '../ir-schema/input-limits.js';
// ---------------------------------------------------------------------------
 ...
 * line up one-for-one.
 */
function chunkByteOffsets(script) {
    const offsets = [];
...
export class ScriptVM {
     ...
     */
    assertScriptBytesUnderLimit(script) {
        if (script.length > InputLimits.MAX_SCRIPT_BYTES) {
            throw new CanonicalJsonError('bytes', `ScriptVM: script length ${script.length} exceeds InputLimits.MAX_SCRIPT_BYTES (${InputLimits.MAX_SCRIPT_BYTES})`, { limit: InputLimits.MAX_SCRIPT_BYTES, actual: script.length });
        }
    }
    // -------------------------------------------------------------------------
     ...
     */
    load(unlockingScript, lockingScript) {
        this.assertScriptBytesUnderLimit(unlockingScript);
        this.assertScriptBytesUnderLimit(lockingScript);
        this.loadInternal(unlockingScript, lockingScript);
    }
     ...
     */
    loadHex(unlockingScriptHex, lockingScriptHex) {
        this.load(unlockingScriptHex ? hexToBytes(unlockingScriptHex) : new Uint8Array(0), hexToBytes(lockingScriptHex));
    }
    /**
     ...
     */
    get lastCodeSeparator() {
        const spend = this.spend;
        if (!spend || spend.lastCodeSeparator === null)
            return -1;
    ...
    // Internal
    // -------------------------------------------------------------------------
    loadInternal(unlockingScript, lockingScript) {
    ...
    afterStep() {
        ...
        if (depth > this._maxStackDepth)
            this._maxStackDepth = depth;
...
//# sourceMappingURL=script-vm.js.map